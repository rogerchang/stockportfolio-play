...
object StockPortfolioValuationActor {
   ...
   */
    def props(stockDirectoryActor : ActorRef) : Props = Props(
        ...
        with AskTimeoutProvider with WrappedMessageProducer)
...
private object StockQuoteRetrieverActor {
   def props(stockDirectoryActor : ActorRef) : Props = Props(
     new StockQuoteRetrieverActor(stockDirectoryActor) with AskTimeoutProvider with WrappedMessageProducer)

  def clusteredProps(receptionistActor : ActorRef) : Props = Props(
    new StockQuoteRetrieverActor(receptionistActor) with AskTimeoutProvider with ClusterClientMessageProducer)
  ...
  this : AskTimeoutProvider with WrappedMessageProducer =>
      ...
      def receive : Receive = {
         ...
         case RetrieveStockQuoteForTicker(stock) => {
          println("In StockQuoteRetrieverActor, received message RetrieveStockQuoteForTicker")
           println("StockDirectory actor = " + stockDirectory)
          val stockQuote = for {
   ...
   */
    def getStockQuotesFuture(stockPortfolio : StockPortfolio) = {
      ...
      val stockQuoteFutures  = (stockPortfolio.portfolioElements.map {
        case (stock, _) =>  {
          println("Sending RetrieveStockQuoteForTicker to internal stockQuoteRetrieverActor")
    ...
    def receive : Receive = {
       ...
       case GetPortfolioValuation(stockPortfolio) => {
         println("Received GetPortfolioValuation with stockPortfolio")
