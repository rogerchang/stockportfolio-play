...
object StockDataRetrieverActor {
  ...
  private trait DefaultStockDataRetrieverProvider extends StockDataRetrieverProvider {
   ...
   private val lookback = config.getInt("server_app.stock.data.retrieve.days.lookback")
    ...
    def receive = {
      case RetrieveStockData(stock, originator) => {
          ...
          println("StockDataRetrieverActor received message RetrieveStockData for stock " + stock)
          val stockData = dataRetriever.retrieve(stock, lookback)
          println("Retrieved stock data information, sending back to sender " + sender)
