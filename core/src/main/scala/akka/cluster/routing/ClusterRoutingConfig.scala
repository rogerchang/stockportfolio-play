...
private[akka] class ClusterRouterGroupActor(val settings: ClusterRouterGroupSettings)
  ...
  override def addRoutees(): Unit = {
    @tailrec
    def doAddRoutees(): Unit = selectDeploymentTarget match {
      case None ⇒ { println ("doAddRoutees:  No more routees to add")}// done
      case Some((address, path)) ⇒
        ...
        println("doAddRoutees: For address " + address + " and path " + path + " adding routee " + routee)
  ...
  def selectDeploymentTarget: Option[(Address, String)] = {
    ...
    val currentNodes = availableNodes
    if (currentNodes.isEmpty || currentRoutees.size >= settings.totalInstances) {
      println("In selectDeploymentTarget:  no deployment target found")
      None
    } else {
      // find the node with least routees
      val unusedNodes = currentNodes filterNot usedRouteePaths.contains
      if (unusedNodes.nonEmpty) {
        println("In selectDeploymentTarget:  with unused nodes, found node " + unusedNodes.head + " with path " + settings.routeesPaths.head)
        ...
        println("In selectDeploymentTarget:  with no unused nodes, found  " + settings.routeesPaths.collectFirst { case p if !used.contains(p) ⇒ (address, p) })
...
private[akka] trait ClusterRouterActor { this: RouterActor ⇒
  ...
  def isAvailable(m: Member): Boolean = {
    println("In isAvailable:  member " + m + ", is it available? " +
  ...
  def availableNodes: immutable.SortedSet[Address] = {
    ...
    //use my own node, cluster information not updated yet
      ...
      println("Available nodes = " + currentNodes)
      currentNodes
  }
  ...
  def addMember(member: Member): Unit = {
    ...
    println("In addMember: nodes is now " + nodes)
    addRoutees()
    println("In addMember:  after addRoutees, routees is now " + cell.router.routees)
    println("In addMember:  after addRoutees, routees addresses is now " + cell.router.routees.map(fullAddress(_)))
  ...
  def removeMember(member: Member): Unit = {
    ...
    println("In removeMember: nodes is now " + nodes)
    ...
    println("In removeMember:  after removeRoutees and addRoutees, routees is now " + cell.router.routees)
    println("In removeMember:  after remoeRoutees and addRoutees, routees addresses is now " + cell.router.routees.map(fullAddress(_)))
  }
