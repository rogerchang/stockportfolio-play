...
final case class ConsistentHashingRoutingLogic(
  ...
  override def select(message: Any, routees: immutable.IndexedSeq[Routee]): Routee = {
    println("In ConsistentHashing.select, for message " + message + " searching routees " + routees)
    if (routees.isEmpty) {
      println("Routees are empty, returning no routee")
      NoRoutee
    }
    else {
      ...
      message match {
        case _ if hashMapping.isDefinedAt(message) ⇒ {
          println("IN ConsistentHashMapping.select, message is defined, returning target " + target(hashMapping(message)))
          target(hashMapping(message))
        }
        case hashable: ConsistentHashable ⇒ {
           println("IN ConsistentHashMapping.select, message has a hash itself, returning target " +  target(hashable.consistentHashKey))
           target(hashable.consistentHashKey)
        }
        case other ⇒
