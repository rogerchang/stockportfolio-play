...
import akka.cluster.{ClusterActorRefProvider, Cluster, Member, MemberStatus}
...
class ClusterClient(
  ...
  def establishing: Actor.Receive = {
    case Contacts(contactPoints) ⇒
      if (contactPoints.nonEmpty) {
        println("Received contacts message")
  ...
  def sendGetContacts(): Unit = {
    println("In sendGetContacts with contacts " + contacts)
    ...
    else contacts foreach { println("Sending GetContacts"); _ ! GetContacts }
  }
}
...
class ClusterReceptionistExtension(system: ExtendedActorSystem) extends Extension {
  ...
  private val receptionist: ActorRef = {
    if (isTerminated)
      ...
      println("Receptionist started with name " + name)
      // important to use val mediator here to activate it outside of ClusterReceptionist constructor
      val mediator = pubSubMediator
      val actorRef = system.actorOf(ClusterReceptionist.props(mediator, role, numberOfContacts,
        responseTunnelReceiveTimeout), name)
      println("Full actorref of started receptionist = " + actorRef)
      println("Starting on system with provider " + system.provider);
      //println("Settings of ClusterActorRefProvider = " + system.provider.asInstanceOf[ClusterActorRefProvider].settings);
    ...
    class ClientResponseTunnel(client: ActorRef, timeout: FiniteDuration) extends Actor {
      ...
      def receive = {
        case Ping                 ⇒ {
          ...
          println("ClientResponseTunnel received a Ping")
        }
        case ReceiveTimeout       ⇒ {
          context stop self
          println("ClientResonseTunnel with ReceiveTimeout")
        }
        ...
        case msg                  ⇒ {
          client forward msg
          println("Forwarding message " + msg + " to client " + client)
        }
      }
    }
  }
...
class ClusterReceptionist(
  ...
  override def preStart(): Unit = {
    ...
    println("ClusterReceptionist is starting up with address " + self.path + " with address " + self.path.address + ", actor ref = " + self)
  }
  ...
  def responseTunnel(client: ActorRef): ActorRef = {
    println("Creating a response tunnel")
    val encName = URLEncoder.encode(client.path.toSerializationFormat, "utf-8")
    println("Looking for response tunnel under name " + encName)
    context.child(encName) match {
      case Some(tunnel) ⇒ {
        println("Found a tunnel " + tunnel)
        tunnel
      }
      case None ⇒ {
        println("Creating a ClientResponseTunnel ")
  ...
  def receive = {
    case msg @ (_: Send | _: SendToAll | _: Publish) ⇒
      ...
      println("Receptionist received message " + msg + " with sender " + sender)
      pubSubMediator.tell(msg, tunnel)

    case GetContacts ⇒ {
      println("GetContacts message received")
