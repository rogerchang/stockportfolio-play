...
class DistributedPubSubMediator(
  ...
  override def preStart(): Unit = {
    ...
    println("DistributedPubSubMediator started")
  }
  ...
  def receive = {
    ...
    case Send(path, msg, localAffinity) ⇒ {
      println("DistributedPubSubMediator: For self address " + selfAddress + " registry has " +
          registry(selfAddress) + " which has content " + registry(selfAddress).content + " and at path " +
      path + " we find " + registry(selfAddress).content.get(path) )
      registry(selfAddress).content.get(path) match {
        ...
        case _ ⇒ {
          ...
          println("DistributedPubSubMediator:  This maps to routees " + routees)
          println("DistributedPubSubMediator:  The full registry = " + registry)
          if (routees.nonEmpty)
    ...
    case Put(ref: ActorRef) ⇒ {
      println("DistributedPubSubMediator:  Received Put message for ref " + ref)
    ...
    case Status(otherVersions) ⇒ {
      println("DistributedPubSubMediator:  Received otherVersions from Status: " + otherVersions)
    ...
    case Delta(buckets) ⇒ {
      ...
      println("DistributedPubSubMediator:  Received Delta message with buckets " + buckets)
    }
  ...
  def put(key: String, valueOption: Option[ActorRef]): Unit = {
    println("DistributedPubSubMediator:  Trying to put actor " + valueOption + " under key " + key + " for self address" + selfAddress)
  ...
  def gossipTo(address: Address): Unit = {
    println("DistributedPubSubMediator:  Gossiping versions " + myVersions + " to actor " + self.path.toStringWithAddress(address))
